...
const ToolsPalette: React.FC<ToolsPaletteProps> = ({ activeTool, onToolSelect }) => {
  const tools: { id: Tool; label: string; icon: React.ReactElement; title: string }[] = [
    ...
    { id: 'image', label: 'Изображение', icon: <DocumentDuplicateIcon />, title: "Добавить изображение: Добавьте новое изображение на холст как отдельный слой." },
    { id: 'transform', label: 'Трансформ', icon: <TransformIcon />, title: "Трансформация: Выберите слой с изображением для перемещения, изменения размера или вращения." },
